...
def plot_alternative_events(gtf_data, gene_id, alt_promoters, alt5sites, alt3sites, alt_ends, exon_ranges_below_one, nintrons, min_start, ncoverage, strand, fig, max_transcripts=30,fscale=10,gs_pos=[0,0]):
    """
    Plot the splicing events for the transcripts of a gene, showing transcripts grouped by alternative promoter usage.
    ...
    :param max_transcripts: Maximum number of transcripts to plot.
    """
    transcript_data = gtf_data[(gtf_data['gene_id'] == gene_id) & (gtf_data['feature'] == 'exon')].groupby('transcript_id').apply(lambda x: x[['start', 'end']].values.tolist())
    ...
    for start, end in exon_ranges_below_one:
        ...
        normalized_end = end
        ce_rect = patches.Rectangle((normalized_start, 0), normalized_end - normalized_start, len(sorted_transcript_data)-1, color='red', alpha=0.1,lw=0.5*.75)
        ax.add_patch(ce_rect)
    # Highlighting introns
    ...
    for i, (transcript_id, exons) in enumerate(sorted_transcript_data):
        ...
        transcript_start = min([exon[0] for exon in exons])
        transcript_end = max([exon[1] for exon in exons])
        ax.plot([transcript_start+5, transcript_end-5], [i, i], color=promoter_color, lw=1*.75)

        # Drawing the exons as rectangles
        for exon in exons:
            exon_start, exon_end = exon
            #print(exon)
            rect = patches.Rectangle((exon_start, i-.1), exon_end-exon_start, 0.2, color=promoter_color,lw=0.5*.75)
            ax.add_patch(rect)
    ...
    ax.set_ylabel('Transcripts',fontsize=font_scale_ax1*3)
    ax.set_xlim(min(transcript_data.explode().min())-100, max(transcript_data.explode().max())+100)  # Set x-axis limits
    ax.set_title(gene_id+' AS Events',fontsize=font_scale_ax1*3)
    return ax
...
def plot_promoter_group(promoter_group_data, promoter_group_id_to_plot, gtf_data, gene_id, asplot = True, covplot=False, save_path=None,fscale1=10,fscale2=10):
    """
    Plots the alternative events and coverage for a specified promoter group.

    Parameters:
        promoter_group_data: List of dictionaries containing promoter group data.
        promoter_group_id_to_plot: The ID of the promoter group to plot.
        gtf_data: DataFrame containing GTF data.
        gene_id: The gene ID.
        save_path: Path to save the figure. If None, the figure will be displayed.
        fscale1: Scaling factor for the alternative events plot.
        fscale2: Scaling factor for the coverage plot.
    """
    # Set up font
    ...
    data = None
    for d in promoter_group_data:
        if d['promoter_group_id'] == promoter_group_id_to_plot:
            data = d
            break
    if data is None:
    ...
    nintrons = data['nintrons'] if 'nintrons' in data else []
    gene_strand = data['gene_strand']
    # Create the figure and GridSpec
    fig = plt.figure(figsize=(8, 11))
    gs = gridspec.GridSpec(6, 12, height_ratios=[1,1,1,1,1,1], hspace=.75,wspace=.2, figure=fig)  # Example: [4, 1, 2] for 3 rows
    gs_pos1=gs[0,2:9]
    gs_pos2=gs[1,0:7]

    # Add text annotations
    fig.text(0.1, 0.90, 'A', fontsize=16, fontweight='bold', va='top', ha='right', transform=fig.transFigure)
    fig.text(0.1, 0.78, 'B', fontsize=16, fontweight='bold', va='top', ha='right', transform=fig.transFigure)

    # Plot Alternative Events
    #ax_ae = fig.add_subplot(gs[0])
    #plt.sca(ax_ae)  # Set current axes
    ...
    if asplot:
        plot_alternative_events(
                            gtf_data, gene_id, {start_pos: transcripts}, alt5sites, alt3sites, alt_ends,
                            exon_ranges_below_one, nintrons, min_start, ncoverage, gene_strand, fig,
                            max_transcripts=30, gs_pos=gs_pos1, fscale=fscale1
                        )
    #ax_ae.set_title(f"Alternative Events for Promoter Group {promoter_group_id}")
    ...
    if covplot:
        plot_cov_splice_sites(
                            gene_id, ncoverage, alt5sites, alt3sites, alt_ends, exon_ranges_below_one,
                            nintrons, npeaks, nvalleys, min_start, fig, gs_pos=gs_pos2, fscale=fscale2
                        )
...
def get_splicing_events_V5(gtf_data, gene_id, gtf=True):
    ...
    if gene_strand == "-":
        promoter_group_ids = {start_pos: idx for idx, start_pos in enumerate(sorted(alternative_promoters.keys(), reverse=True))}
    else:
    ...
    for start_pos, transcripts in alternative_promoters.items():
        ...
        #were we have multiple single exon transcripts
        if len(promoter_exons['transcript_id'].unique()) == len(promoter_exons['exon_identifier'].unique()):
            ...
            for _, row in promoter_exons.iterrows():
                ...
                for transcript_id in transcripts:
                    if transcript_id not in transcript_data:
                        transcript_data[transcript_id] = {
                            'gene_id': gene_id,
                            'ap': 0, 'apt': len(alternative_promoters), 'ce': 0, 'cet': 0, 'a5': 0,
                            'a5t': 0, 'a3': 0, 'a3t': 0,
                            'ae': 0, 'aet': 0, 'i': 0, 'it': 0,
                ...
                if len(alternative_promoters) > 1:
                    for transcript_id in transcripts:
                        #transcript_data[transcript_id]['ap'] += transcript_id in alt_promoter_fract_dist
            ...
            if len(promoter_exons['transcript_id'].unique()) > 1 and len(promoter_exons['exon_identifier'].unique()) > len(promoter_exons['transcript_id'].unique()):
                ...
                for _, row in promoter_exons.iterrows():
                    ...
                    if transcript_id not in transcript_data:
                        transcript_data[transcript_id] = {
                            'gene_id': gene_id,
                            'ap': 0, 'apt': len(alternative_promoters), 'ce': 0, 'cet': len(exon_ranges_below_one), 'a5': 0,
    ...
    #Assign exon numbers for the gene in labeled_gtf_data
    exons = labeled_gtf_data[labeled_gtf_data['feature'].str.contains('exon')]
    if not exons.empty:
        # Identify all unique exons within the gene based on start and end positions
        unique_exons = exons[['start', 'end']].drop_duplicates().copy()
    
        # Sort unique exons based on strand orientation
        if gene_strand == '+':
            unique_exons = unique_exons.sort_values(by=['start', 'end'])
        else:
            unique_exons = unique_exons.sort_values(by=['end', 'start'], ascending=[False, False])
    
        # Assign exon numbers sequentially
        unique_exons = unique_exons.reset_index(drop=True)
        unique_exons['exon_number'] = unique_exons.index + 1  # Exon numbering starts at 1
    
        # Create a mapping from (start, end) to exon_number
        exon_num_mapping = unique_exons.set_index(['start', 'end'])['exon_number'].to_dict()
    
        # Assign gene-wide exon numbers to all exons in labeled_gtf_data
        labeled_gtf_data.loc[labeled_gtf_data['feature'].str.contains('exon'), 'exon_number'] = labeled_gtf_data.apply(
            lambda row: exon_num_mapping.get((row['start'], row['end'])),
            axis=1
        )
    
        # Optional: If you no longer need transcript-specific exon numbers, you can remove the following lines
        # If you wish to keep both gene-wide and transcript-specific exon numbers, consider using separate columns
        # For this example, we'll remove transcript-specific exon numbering
        # Uncomment the following lines if you want to remove transcript-specific exon_number
        # labeled_gtf_data = labeled_gtf_data.drop(columns=['transcript_specific_exon_number'], errors='ignore')
    
        # Update the 'attribute' field to include the gene-wide 'exon_number'
        for idx, row in labeled_gtf_data.iterrows():
            if 'exon' in row['feature']:
                exon_number = row.get('exon_number', None)
                # Parse the attribute field into a dictionary
                attributes = parse_gtf_attributes(row['attribute'])
                # Assign the gene-wide exon_number
                if exon_number is not None:
                    attributes['exon_number'] = str(exon_number)
                # Reconstruct the attribute string
                attribute_string = "; ".join(f'{key} "{value}"' for key, value in attributes.items()) + ";"
                labeled_gtf_data.at[idx, 'attribute'] = attribute_string
    else:
        labeled_gtf_data['exon_number'] = None
    # After processing all promoter groups
    if not gtf:
        labeled_gtf_data = pd.DataFrame()  # Return an empty DataFrame if gtf is False
        
    labeled_gtf_data.drop(columns=['attribute'], inplace=True)
...
def main():
    parser = argparse.ArgumentParser(description="Process GTF data to identify and analyze splicing events.")
    parser.add_argument("gtf_file", help="Path to the GTF file")
    parser.add_argument("tpm_file", help="Path to the TPM count file (TSV format)")
    parser.add_argument("-o", "--output_prefix", default="output", help="Prefix for output files")
    parser.add_argument("-p", "--num_processes", type=int, default=8, help="Number of processes for multiprocessing")
    parser.add_argument("-g", "--gene_list",  help="Optional: Path to a file containing a list of gene IDs to process (one ID per line)")
    parser.add_argument("--skip_gtf_output", action="store_true", help="Skip the generation of the labeled GTF output file")
    ...
    # Filter gene IDs if a list is provided
    if args.gene_list:
        with open(args.gene_list, "r") as f:
            gene_ids_to_process = [line.strip() for line in f]
        gtf_data = gtf_data[gtf_data['gene_id'].isin(gene_ids_to_process)]
    ...
    if args.gene_list:
        ...
        gene_ids = gtf_data['gene_id'].unique()
    
    args_list = [(gtf_data, gene_id, not args.skip_gtf_output) for gene_id in gene_ids]
    total_gene_ids = len(args_list)

    #with Pool(processes=args.num_processes) as pool:
    #    results = pool.starmap(get_splicing_events_V5, tqdm(args_list, total=total_gene_ids, desc="Identifying splicing events"))
    # Define batch size (adjust based on performance)
    
    
    batch_size = 100*args.num_processes  # Batch size to prevent stalling during multiprocessing extremely large files

    results = []
    with Pool(processes=args.num_processes) as pool:
        for batch in tqdm(list(chunk_list(args_list, batch_size)), total=total_gene_ids // batch_size + 1, desc="Processing Batches"):
            batch_results = pool.starmap(get_splicing_events_V5, tqdm(batch,total=len(batch),desc="Identifying splicing events"))  # Process each batch
            results.extend(batch_results)  # Collect results


    # Aggregate results
    transcript_results = {}
    gtf_results = pd.DataFrame()
    promoter_group_results = []

    for transcript_data, labeled_gtf_data, promoter_group_data in results:
        transcript_results.update(transcript_data)
        if not args.skip_gtf_output:
            gtf_results = pd.concat([gtf_results, labeled_gtf_data])
        promoter_group_results.extend(promoter_group_data)

    # Save promoter group data
    promoter_group_output = f"{args.output_prefix}_promoter_group_data.pkl"
    with open(promoter_group_output, 'wb') as file:
        pickle.dump(promoter_group_results, file)

    # Save labeled GTF data (if not skipped)
    if not args.skip_gtf_output:
        gtf_output = f"{args.output_prefix}_labeled.gtf"
        gtf_results.to_csv(gtf_output, sep="\t", index=False, header=True)

    # Save splicing events transcript data
    transcript_df = pd.DataFrame.from_dict(transcript_results, orient='index')
    transcript_df.reset_index(inplace=True)
    transcript_df.rename(columns={'index': 'transcript_id'}, inplace=True)
    transcript_df['ids'] = transcript_df['transcript_id'] + '_' + transcript_df['gene_id']
    transcript_df.set_index('ids', inplace=True)

    # Load TPM count data
    rna_counts = pd.read_csv(args.tpm_file, sep="\t")
    rna_counts.set_index('ids', inplace=True)

    # Filter transcript data based on TPM counts
    transcript_df = transcript_df.loc[transcript_df.index.intersection(rna_counts.index)]

    # Save filtered transcript data
    transcript_output = f"{args.output_prefix}_AS_events.csv"
    transcript_df.to_csv(transcript_output, sep="\t")
